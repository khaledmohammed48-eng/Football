            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {table.map((row, i) => {
                  ...
                  const isTop3 = rank <= 3;
                  return (
                    <tr key={row.id} className={`transition ${rank === 1 ? 'bg-yellow-50' : rank === 2 ? 'bg-gray-50/60' : rank === 3 ? 'bg-orange-50/40' : 'hover:bg-gray-50'}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                            {row.logoUrl
                              ? <img src={row.logoUrl} alt={row.name} className="w-8 h-8 object-cover rounded-full" />
                              : <span className="text-sm">🏟️</span>}
                          </div>
                          <div>
                            <div className={`font-semibold ${isTop3 ? 'text-gray-900' : 'text-gray-700'}`}>{row.name}</div>
                      ...
                      <td className="px-3 py-3 text-center">
                        <span className={`font-medium ${row.goalDiff > 0 ? 'text-green-600' : row.goalDiff < 0 ? 'text-red-600' : 'text-gray-500'}`}>
                          {row.goalDiff > 0 ? '+' : ''}{row.goalDiff}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-center">
                        <span className={`inline-block min-w-[32px] text-center font-black text-base rounded-lg py-0.5 px-2 ${
                          rank === 1 ? 'text-yellow-700 bg-yellow-100' :
                          rank === 2 ? 'text-gray-600 bg-gray-100' :
                          rank === 3 ? 'text-orange-700 bg-orange-100' :
